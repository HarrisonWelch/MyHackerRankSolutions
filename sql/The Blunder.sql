...
-- difference between her miscalculation (using salaries with any zeroes 
-- removed), and the actual average salary.

-- Write a query calculating the amount of error (i.e.: actual - miscalulated average monthly 
-- salaries), and round it up to the next integer.

SELECT
    CEIL(AVG(SALARY) - -- avg the salaries 
            AVG(REPLACE(SALARY,'0',''))) -- avg them w/out zeroes
FROM
    EMPLOYEES;